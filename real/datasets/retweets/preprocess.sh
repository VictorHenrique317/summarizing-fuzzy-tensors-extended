...
ARGIND == 2 { team[$1] = $2 }
ARGIND == 3 { $4 *= coeff[$3]; if ($3 in team) $3 = team[$3]; print; $4 = 1 / (1 + exp(.5 * (7 - $4))); print > "3d/influences" }' - teams 3d/retweets 2> /dev/null | LC_ALL=C sort -k 4gr | head -1000 | cut -d ' ' -f -3 > 3d/init_patterns
...
ARGIND == 2 { team[$1] = $2 }
ARGIND == 3 { $3 *= coeff[$1]; if ($1 in team) $1 = team[$1]; print; $3 = 1 / (1 + exp(.04166666666666667 * (84 - $3))); print > "2d/influences" }' - teams 2d/retweets 2> /dev/null | LC_ALL=C sort -k 3gr | head -1000 | cut -d ' ' -f -2 > 2d/init_patterns
