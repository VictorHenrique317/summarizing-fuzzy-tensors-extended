...
class RssEvolution:
    ...
    @staticmethod
    def calculateModelRss(dataset:RetweetsDataset, patterns: List[Pattern]):
        ...
        for dims, prediction in prediction_matrix.items():
            ...
            cell_prediction_rss = RssEvolution.calculateCellRSS(dataset_matrix, dims, prediction)
            cell_lambda0_rss = RssEvolution.calculateCellRSS(dataset_matrix, dims, dataset.getDensity())
    ...
    @staticmethod
    def calculate(dataset, patterns:List[Pattern], max_pattern_nb=20, empty_model=False):
