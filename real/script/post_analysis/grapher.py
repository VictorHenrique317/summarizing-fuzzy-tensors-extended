...
class Grapher():
    def __init__(self, configuration_name, dataset, calculate_rss_evolution=False, calculate_quality=False) -> None:
        ...
        self.__experiment_analysis = ExperimentAnalysis(self.__configuration_name, dataset)
    ...
    def __initialize(self, calculate_rss_evolution, calculate_quality):
        ...
        nclusterboxnosubfibermaximization = algorithms[5]
    ...
    def __configureCurve(self, x, y, curve, xlabel, ylabel, title, scatter=True, invert_x=True,
    ...
    def __drawRssEvolutionGraphs(self, save, figure_width, figure_height, folder):
        for algorithm in Configs.getAlgorithms():
            ...
            plotting_data_filename = f"real-{self.__dimension}d-rssevolution#" \
    ...
    def __drawNormalGraphs(self, save, figure_width, figure_height, folder): # ARRUMAR
        ...
        for algorithm in Configs.getAlgorithms():
            ...
            plotting_data_filename = f"real-{self.__dimension}d-{self.__attribute.value.lower().replace(' ','')}" \
                                        f"#{algorithm.lower().replace(' ','')}-"
            self.__savePlottingData(x, y, folder, plotting_data_filename)
